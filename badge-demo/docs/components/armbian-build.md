...
The current immediate use is rebuilding the BeagleBadge `vendor-edge` kernel packages so the staged `I2C_SLAVE_TESTUNIT` and `I2C_SLAVE_EEPROM` config changes can be boot-tested.
...
3. Commit the superproject pointer update.
4. Run the top-level x86 Docker wrapper to produce test kernel packages.
5. Preserve current BeagleBadge state with `./scripts/capture_beaglebadge_state.sh` before discussing any fresh image replacement.
