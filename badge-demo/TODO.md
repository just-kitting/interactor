...
- [ ] Rebuild the 6-patch ISR-branch diagnostic kernel after the corrected `0006` patch payload is in place
...
- [X] Add a repeatable J7-to-J6 target-mode validation harness
...
- [X] Build and install the 11-patch write-transaction-state follow-up kernel
- [X] Reboot into the 11-patch write-transaction-state follow-up kernel
...
- [X] Reboot into the 13-patch slave-TX-trace follow-up kernel
- [X] Capture `slave tx-requested` / `slave tx-processed` logs during the proc-call tests
- [X] Prove that J6 already generates `0x04 0x03 0x02 0x01` during the proc-call read phase
- [X] Narrow the remaining proc-call gap to first-byte handling at repeated-start read address match
- [X] Stage a 14-patch follow-up that primes the first slave TX byte on read address match
- [X] Build and install the 14-patch first-byte-priming follow-up kernel
...
- [X] Install and reboot into `P3659`
...
- [X] Build and copy back a distinct kernel artifact from the cleaned recv-len state (`3c92960` / `e49bac3`)
- [X] Install and reboot into the cleaned recv-len artifact
