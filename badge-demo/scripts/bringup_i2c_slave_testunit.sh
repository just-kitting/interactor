...
slave_node="$bus-$(printf '%04x' $((0x1000 + addr_dec)))"

device_bound() {
  [ -L "/sys/bus/i2c/devices/$slave_node/driver" ]
}
...
case "$subcmd" in
  start)
    ...
    if [ ! -e "/sys/bus/i2c/devices/$slave_node" ]; then
      echo "slave-testunit instantiation did not create $slave_node" >&2
      exit 1
    fi
    if ! device_bound; then
      echo "slave-testunit instantiated at $slave_node but driver did not bind" >&2
      echo "check dmesg for adapter support errors such as i2c_slave_register: not supported by adapter" >&2
      exit 1
    fi
  ...
  stop)
    ...
    if [ ! -e "/sys/bus/i2c/devices/$slave_node" ]; then
      echo "no slave-testunit device at $slave_node"
      exit 0
    fi
  ...
  status)
    if [ -e "/sys/bus/i2c/devices/$slave_node" ]; then
      echo "present: $slave_node"
      if device_bound; then
        echo "bound: yes"
      else
        echo "bound: no"
      fi
      if [ -r "/sys/bus/i2c/devices/$slave_node/name" ]; then
        cat "/sys/bus/i2c/devices/$slave_node/name"
      fi
    else
      echo "absent: $slave_node"
      exit 1
    fi
    ;;
  *)
